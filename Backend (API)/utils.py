...
# Load spaCy model (assumes en_core_web_sm is pre-installed)
def load_spacy_model():
    try:
        return spacy.load("en_core_web_sm")
    except OSError:
        raise RuntimeError("spaCy model 'en_core_web_sm' not found. Ensure it's installed via requirements.txt.")


# Initialize sentiment analyzer (assumes vader_lexicon is pre-installed)
def get_sentiment_analyzer():
    try:
        return SentimentIntensityAnalyzer()
    except LookupError:
        raise RuntimeError("NLTK vader_lexicon not found. Ensure it's pre-installed via Dockerfile.")
